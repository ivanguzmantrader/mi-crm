...
- **Móvil:** tab bar inferior fija de hasta **4 ítems** (Hoy · Clientes · Ventas · Equipo). "Equipo" solo se muestra si el rol es dueña; el resto de personas ve 3 ítems. El acceso a "Mi cuenta" va aparte, vía avatar, y no ocupa ítem de la tab bar. Altura ~64px, cada ítem ≥56px de alto táctil.
- **Escritorio:** barra lateral fija (240px), con el mismo acceso a "Mi cuenta" al pie; contenido centrado a 1200px.
...
`radius:9999px; padding:5px 12px; font-size:13px; font-weight:500`. Punto de 7px del color `fg`.
- **Estado del cliente** (calculado siempre a partir de sus ventas, nunca editable a mano): Nuevo lead=info · En negociación=primary-subtle/primary · Ganado=success · Perdido=error.
- **Estado del seguimiento** (no es un campo propio; se deriva de la fecha y de si está hecho): Atrasado=error · Pendiente (en Ficha) / Hoy (en Tareas del día)=warning/neutral · Completado=primary-subtle.
...
### Tab bar (móvil)
Contenedor `surface`, borde superior 1px, hasta 4 ítems en flex (ver Navegación). Ítem: icono 22px + label 11px. Activo `--color-primary`/600; inactivo `--color-text-subtle`/500. Cada ítem ≥56px alto.
...
- [ ] En móvil, navegación en tab bar inferior de hasta 4 ítems.
